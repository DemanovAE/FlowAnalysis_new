...
void PictDiffParAntVsPtPID(const Char_t *inFileName,const Int_t energy = 28){
    ...
    Float_t size[]={1.8, 1.8, 1.8, 1.8};
  
    std::vector<Double_t> AxisUp   = {-0.01,0.31};
    std::vector<Double_t> AxisDown = {-0.012,0.025};

    std::vector<MyDrawObject *> drawOdj;
    std::vector<AnalysisFunction *> AnalysOdj;
    std::vector<Int_t> CentMin={7,4,0,2};
    std::vector<Int_t> CentMax={8,6,3,8};
    ...
    std::vector<std::vector<double>> ptBin         = {
        {0.5,0.6,0.8,1.0,1.2,1.4,1.6,1.8,2.0,2.4,2.8,3.2},
        {0.5,0.6,0.8,1.0,1.2,1.6,2.0,2.6,3.2},
        {0.5,0.6,0.8,1.0,1.2,1.6,2.0,2.6,3.2},
        {0.6,1.0,1.4,2.0,2.6,3.2},
        {0.4,0.6,0.8,1.2,1.6,2.0,2.6,3.2},
    };
    ...
    for(Int_t pid=0; pid<4; pid++){
        for(Int_t ih=0; ih<fNHarmonic.size(); ih++){    
            for(Int_t ch=0; ch<2; ch++){
                ...
                drawOdj.back()->SetMarkerStyle(style[ch+ih*2]);
                ...
                drawOdj.back()->SetDrawOdject( AnalysOdj.back()->FlowVsPt_EtaSub(fileIn,
                        ...
                        Form("tp2_v%iewTPC%sEta%iTPCandTOF",fNHarmonic[ih],fPidCode[3*2+ch].Data(),(int)(fEtaGap[0]*100)),
                        Form("tp2_meanPtV%i%sEta%iTPCandTOF",fNHarmonic[ih],fPidCode[3*2+ch].Data(),(int)(fEtaGap[0]*100)),
                        Form("gt_v%i_%s_pt",fNHarmonic[ih],fPidCode[pid*2+ch].Data()),
                        CentMin[pid],CentMax[pid], ptBin[ih]) );
            ...
            drawOdj.push_back(new MyDrawObject());
            drawOdj.back()->SetMarkerColor(color[ih]);
            drawOdj.back()->SetMarkerStyle(style[ih*2]);
            drawOdj.back()->SetMarkerSize(size[pid]);
    ...
    canvas->SetTextInfo(Form("#font[42]{ #scale[1.0]{ Centrality %i-%i %%, p#bar{p} }}",cent[CentMax[0]+1], cent[CentMin[0]]),"Pad", 0, 0.1, 0.8*AxisUp[1], 0.07, 0.0);
    canvas->SetTextInfo(Form("#font[42]{ #scale[1.0]{ Centrality %i-%i %%}}",cent[CentMax[1]+1], cent[CentMin[1]]),"Pad", 1, 1, 0.9*AxisUp[1], 0.07, 0.0);
    canvas->SetTextInfo(Form("#font[42]{ #scale[1.0]{ Centrality %i-%i %%}}",cent[CentMax[2]+1], cent[CentMin[2]]),"Pad", 2, 1, 0.9*AxisUp[1], 0.07, 0.0);
    canvas->SetTextInfo(Form("#font[42]{ #scale[1.0]{ Centrality %i-%i %%}}",cent[CentMax[3]+1], cent[CentMin[3]]),"Pad", 3, 1, 0.9*AxisUp[1], 0.07, 0.0);
    ...
    result->SaveAs(Form("./pict2/PictDiffParAntVsPt_%i_%i_%i.png",energy,cent[CentMax[0]+1], cent[CentMin[0]]));
